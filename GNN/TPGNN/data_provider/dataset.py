...
def transform_time(data: torch.Tensor, train: bool, opt: object, start: int, time_stamp: torch.Tensor) -> torch.Tensor:
    ...
    if opt.mode == 1:
        ...
        #########################
        print(f"train: {train}, day_slot: {day_slot}, n_his: {n_his}, n_pred: {n_pred}, T4N_step: {T4N_step}")
        print(f"n_slot (calculated): {n_slot}")
        print(f"len(data): {len(data)}, day_slot: {day_slot}, n_day: {len(data) // day_slot}")
