...
class Trainer():
    ...
    def train(self, input, real_val, idx=None):
        ...
        # Fix: Only use the first feature dimension if real_val has multiple features
        if real_val.dim() == 4 and real_val.size(1) > 1:
            real_val = real_val[:, 0, :, :]  # Select only first feature: [batch, nodes, time]
        ...
        # Ensure task_level doesn't exceed the actual sequence length
        effective_task_level = min(self.task_level, self.seq_out_len)
        
        if self.cl:
            loss = self.loss(predict[:, :, :, :effective_task_level], real[:, :, :, :effective_task_level], 0.0)
        else:
